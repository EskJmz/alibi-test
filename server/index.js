...
function getQuestion(room) {
  ...
  const content = CONTENT[alibiKey];
  const qs = content.questions;
  const connStart = content.connivenceStart || 6;
  const factual = qs.slice(0, connStart);       // questions factuelles
  const conniv  = qs.slice(connStart);           // questions de connivence

  // Nombre de manches déjà jouées par cette équipe
  const teamHistory = room.history.filter(h => h.team === room.currentTeam);
  const teamRounds  = teamHistory.length;

  // Nombre total de manches que cette équipe va jouer
  const totalTeamRounds = room.singleTeam ? room.rounds : Math.ceil(room.rounds / 2);

  // Construire l'ordre des questions pour cette équipe si pas encore fait
  const key = `_qOrder_${room.currentTeam}`;
  if (!room[key]) {
    // Mélanger les factuelles, puis insérer au moins 2 connivence garanties
    const shuffledFactual = [...factual].sort(() => Math.random() - 0.5);
    const shuffledConniv  = [...conniv].sort(() => Math.random() - 0.5);

    let order = [];
    if (totalTeamRounds <= 2) {
      // Toutes les manches sont de connivence
      order = [...shuffledConniv, ...shuffledFactual];
    } else {
      // Placer 2 connivences garanties, le reste en factuel, cycler si besoin
      const guaranteed = shuffledConniv.slice(0, 2);
      const rest = [...shuffledFactual, ...shuffledConniv.slice(2)];
      // Insérer les 2 connivences dans les 2 dernières positions des totalTeamRounds
      const positions = Array.from({ length: totalTeamRounds }, (_, i) => rest[i % rest.length]);
      // Remplacer les 2 dernières par les connivences garanties
      if (totalTeamRounds >= 2) {
        positions[totalTeamRounds - 1] = guaranteed[0];
        positions[totalTeamRounds - 2] = guaranteed[1];
      }
      order = positions;
    }
    room[key] = order;
  ...
  const qOrder = room[key];
  return qOrder[teamRounds % qOrder.length];
}
...
io.on('connection', (socket) => {
  ...
  socket.on('assign:confirm', () => {
    ...
    room._awarded = false; room._nextReady = new Set();
    room._replayReady = new Set();
    // Réinitialiser les ordres de questions
    room._qOrder_A = null; room._qOrder_B = null;
    ...
    // Envoyer la liste des alibis disponibles au chef pour qu'il choisisse
    const alibiList = Object.keys(CONTENT).map(k => ({ key: k, label: CONTENT[k].label }));
    io.to(room.hostId).emit('alibi:choose', { alibiList, singleTeam: room.singleTeam });

    // Les autres voient un écran d'attente
    roomPlayers(room).filter(p => p.sid !== room.hostId).forEach(p => {
      io.to(p.sid).emit('alibi:waitingChoice', {});
    });
  });

  // ── SÉLECTION ALIBI PAR LE CHEF ──
  socket.on('alibi:select', ({ alibiA, alibiB }) => {
    const room = getRoom(socket.data.roomId);
    if (!room || room.hostId !== socket.id) return;
    if (!CONTENT[alibiA]) return;
    if (!room.singleTeam && !CONTENT[alibiB]) return;
    if (!room.singleTeam && alibiA === alibiB) {
      socket.emit('assign:error', { msg: 'Les deux équipes doivent avoir des alibis différents.' });
      return;
    }

    room.alibiA = alibiA;
    room.alibiB = room.singleTeam ? null : alibiB;
    room._qOrder_A = null; room._qOrder_B = null;

    const teamA = getTeam(room, 'A');
    const teamB = getTeam(room, 'B');
