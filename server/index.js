...
function getQuestion(room) {
  ...
  // Compter combien de fois cette équipe a déjà joué
  const teamRounds = room.history.filter(h => h.team === room.currentTeam).length;
  // Index strict — jamais de doublon, plafonné au nombre de questions dispo
  const idx = teamRounds % qs.length;
  return qs[idx];
}
...
io.on('connection', (socket) => {
  ...
  socket.on('enqueteur:award', ({ point }) => {
    ...
    // Marquer que le point a été accordé/refusé
    room._awarded = true;
  ...
  socket.on('enqueteur:nextQuestion', () => {
    ...
    if (!room) return;
    if (!getEnqueteurs(room).find(p => p.sid === socket.id)) return;
    // FIX : bloquer si le point n'a pas encore été accordé/refusé
    if (!room._awarded) return;
    advanceRound(room);
  });
  ...
  socket.on('enqueteur:nextQuestionEven', () => {
    ...
    const enqs = getEnqueteurs(room);
    if (!enqs.find(p => p.sid === socket.id)) return;
    // FIX : bloquer si la validation n'a pas encore eu lieu
    if (!room._awarded) return;
    // FIX : les deux enquêteurs doivent appuyer
    if (!room._nextReady) room._nextReady = new Set();
    room._nextReady.add(socket.id);
    if (room._nextReady.size < enqs.length) {
      // Notifier l'autre enquêteur qu'on est prêt
