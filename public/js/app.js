...
let _currentAlibiList = [];

function renderAlibiList(containerId, team, alibiList) {
  const container = document.getElementById(containerId);
  if (!container) return;
  _currentAlibiList = alibiList;
  container.innerHTML = '';
  alibiList.forEach(a => {
    const card = document.createElement('div');
    card.className = 'mode-card';
    card.id = `alibi-card-${team}-${a.key}`;
    const title = document.createElement('div');
    title.className = 'mode-title';
    title.textContent = a.label;
    card.appendChild(title);
    card.addEventListener('click', () => selectAlibi(team, a.key));
    container.appendChild(card);
  });
}

function selectAlibi(team, key) {
  if (team === 'A') {
    selectedAlibiA = key;
    _currentAlibiList.forEach(a => {
...
function updateAlibiConfirmBtn() {
  const bSection = document.getElementById('alibi-b-section');
  const singleTeam = !bSection || bSection.style.display === 'none';
  ...
  if (!singleTeam && selectedAlibiA && selectedAlibiB && selectedAlibiA === selectedAlibiB) {
