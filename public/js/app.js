...
function renderPills(containerId, players, targetRole, max) {
  ...
  container.innerHTML = players.map(p => {
    ...
    return `<div class="pill ${cls} ${dis ? 'disabled' : ''}" ${!dis ? `onclick="assignRole('${safeSid}','${newRole}')"` : ''}>
...
document.getElementById('btn-enq-even-submit').addEventListener('click', () => {
  ...
  show('enq-even-sent'); stopClientTimer('enq-even-arc');
});
...
document.getElementById('btn-next-question').addEventListener('click', () => {
  if (L.role === 'enqueteur') {
    if (L.mode === 'odd') socket.emit('enqueteur:nextQuestion');
    else socket.emit('enqueteur:nextQuestionEven');
  }
});

// ── FIN ──
document.getElementById('btn-replay').addEventListener('click', () => { socket.emit('room:startAssign'); });
...
socket.on('room:state', (state) => {
  ...
  const showB = !state.singleTeam;
  ['enq-odd-sb','enq-even-sb','rw-sb','pe-sb','ans-sb','end-sb'].forEach(id => {
    const el = document.getElementById(id);
    if (el) { const card = el.closest('.score-c'); if (card) card.style.display = showB ? '' : 'none'; }
  });
});
...
socket.on('timer:start', ({ seconds, endsAt }) => {
  ...
  const arcMap = {
    ...
    's-enq-even':      ['enq-even-arc',   'enq-even-cd'],
  };
...
socket.on('round:ready', ({ round, team, totalRounds }) => {
  ...
  if (L.role === 'enqueteur') {
    if (L.mode === 'odd') {
      ...
      hide('enq-even-validation');
      hide('enq-even-saisie');
...
socket.on('question:enqueteur:even', ({ question, team, assignedPlayer, round, totalRounds }) => {
  ...
  setText('btn-next-question', 'Question suivante →');
});
...
socket.on('enqueteur:showAnswers', ({ answers, team }) => {
  ...
  answers.forEach(a => {
    ...
    lbl.className = 'label'; lbl.textContent = a.name;
    const box = document.createElement('div');
    box.className = 'ans-box'; box.textContent = a.answer;
...
socket.on('enqueteur:crossValidate', ({ otherEnqName, otherAnswer }) => {
  setText('enq-even-other-lbl', `Réponse de ${escapeHtml(otherEnqName)}`);
  document.getElementById('enq-even-other-ans').textContent = otherAnswer;
...
// ── AFFICHAGE RÉPONSES TOUT LE MONDE ──
...
socket.on('round:showAnswers', ({ team, question, answers, point, scoreA, scoreB }) => {
  ...
  badge.innerHTML = point
    ? `<span class="badge b-green">+1 point</span>`
    : `<span class="badge b-red">0 point</span>`;
  ...
  // Scores
  setText('sa-score-a', scoreA); setText('sa-score-b', scoreB);

  // Bouton question suivante visible seulement pour l'enquêteur
  if (L.role === 'enqueteur') { show('btn-next-question'); hide('sa-wait-next'); }
  else { hide('btn-next-question'); show('sa-wait-next'); }

  // Retour visuel pour les suspects actifs
...
socket.on('game:end', ({ scoreA, scoreB, history }) => {
  ...
  teams.forEach(team => {
    ...
    section.className = 'history-team';
    section.innerHTML = `<div class="history-team-title">Suspects ${team}</div>`;
    items.forEach(item => {
      const div = document.createElement('div');
      div.className = `history-item ${item.point ? 'point' : 'no-point'}`;
