...
// ── SÉLECTION ALIBI ──
let selectedAlibiA = null;
let selectedAlibiB = null;

function renderAlibiList(containerId, team, alibiList) {
  const container = document.getElementById(containerId);
  if (!container) return;
  container.innerHTML = alibiList.map(a => `
    <div class="mode-card" id="alibi-card-${team}-${a.key}" onclick="selectAlibi('${team}', '${a.key}', ${JSON.stringify(alibiList).replace(/'/g, "\'")})">
      <div class="mode-title">${escapeHtml(a.label)}</div>
    </div>`).join('');
}

function selectAlibi(team, key, alibiList) {
  if (team === 'A') {
    selectedAlibiA = key;
    alibiList.forEach(a => {
      const el = document.getElementById(`alibi-card-A-${a.key}`);
      if (el) el.classList.toggle('selected', a.key === key);
    });
  } else {
    selectedAlibiB = key;
    alibiList.forEach(a => {
      const el = document.getElementById(`alibi-card-B-${a.key}`);
      if (el) el.classList.toggle('selected', a.key === key);
    });
  }
  updateAlibiConfirmBtn();
}

function updateAlibiConfirmBtn() {
  const singleTeam = !document.getElementById('alibi-b-section') || document.getElementById('alibi-b-section').style.display === 'none';
  const valid = selectedAlibiA && (singleTeam || (selectedAlibiB && selectedAlibiA !== selectedAlibiB));
  document.getElementById('btn-alibi-confirm').disabled = !valid;
  const err = document.getElementById('alibi-select-err');
  if (selectedAlibiA && selectedAlibiB && selectedAlibiA === selectedAlibiB) {
    err.textContent = 'Les deux équipes doivent avoir des alibis différents.';
...
document.getElementById('btn-alibi-confirm').addEventListener('click', () => {
  socket.emit('alibi:select', { alibiA: selectedAlibiA, alibiB: selectedAlibiB });
});
...
socket.on('alibi:choose', ({ alibiList, singleTeam }) => {
  selectedAlibiA = null; selectedAlibiB = null;
  document.getElementById('alibi-b-section').style.display = singleTeam ? 'none' : '';
  renderAlibiList('alibi-list-a', 'A', alibiList);
  if (!singleTeam) renderAlibiList('alibi-list-b', 'B', alibiList);
  document.getElementById('btn-alibi-confirm').disabled = true;
  go('s-alibi-select');
});

socket.on('alibi:waitingChoice', () => { go('s-alibi-wait-choice'); });
...
socket.on('round:ready', ({ round, team, totalRounds, assignedSuspect }) => {
  ...
  if (L.role === 'enqueteur') {
    if (L.mode === 'odd') {
      ...
      setText('enq-odd-q', `Prêt à interroger les Suspects ${team}`);
      document.getElementById('enq-odd-q').style.borderLeftColor = 'var(--blue)';
      document.getElementById('enq-odd-q').style.color = 'var(--muted)';
      document.getElementById('enq-odd-q').style.fontStyle = 'italic';
      ...
      setText('enq-even-qnum', '');
      // Cacher la zone question tant qu'elle n'est pas lancée
      hide('enq-even-q-card');
      ...
      // Afficher le nom du suspect assigné
      if (assignedSuspect) {
        setText('enq-even-team-lbl', `Suspects ${team} interrogés`);
        setText('enq-even-suspect-name', `Vous interrogez : ${assignedSuspect}`);
        show('enq-even-suspect-name');
      }
      go('s-enq-even');
    }
  } else {
...
socket.on('question:enqueteur', ({ question, team, round, totalRounds }) => {
  ...
  document.getElementById('enq-odd-q').style.borderLeftColor = '';
  document.getElementById('enq-odd-q').style.color = '';
  document.getElementById('enq-odd-q').style.fontStyle = '';
...
socket.on('game:end', ({ scoreA, scoreB, history }) => {
  ...
  // Afficher rejouer seulement pour le chef
  if (L.isHost) { show('btn-replay'); hide('replay-wait'); }
  else { hide('btn-replay'); show('replay-wait'); }
  go('s-end');
  updateScoreVisibility();
});
